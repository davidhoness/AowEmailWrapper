namespace EricDaugherty.CSES.SmtpServer
{
	...
	public class SMTPContext : object
	{
		...
		{
            Trace.WriteLine(string.Format("Connection {0}: New connection from client {1}", connectionId, socket.RemoteEndPoint));
		...
		{
            Trace.WriteLine(string.Format("Connection {0}: Wrote Line: {1}", connectionId, data));
		...
		public String ReadLine()
		{
			...
			do
			{
				...
				if( count == 0 )
				{
                    Trace.WriteLine("Socket closed before end of line received.");
					return null;
				}

				inputBuffer.Append( encoding.GetString( byteBuffer, 0, count ) );
                //Trace.WriteLine(string.Format("Connection {0}: Read: {1}", connectionId, inputBuffer));
			}
		...
		private string ReadBuffer()
		{
			...
			if( inputBuffer.Length > 0 )				
			{
				...
				if( eolIndex != -1 )
				{
                    ...
                    //Trace.WriteLine(string.Format("Connection {0}: Read Line: {1}", connectionId, output));
					return output;
				}				
			}
			return null;
