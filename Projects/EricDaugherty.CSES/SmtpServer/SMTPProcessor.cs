namespace EricDaugherty.CSES.SmtpServer
{
    ...
    public class SMTPProcessor : IDisposable
    {
        ...
        public void ProcessConnection(Socket socket)
        {
            ...
            {
                Trace.WriteLine(string.Format("Connection {0}: Error: {1}", context.ConnectionId, exception.Message));
                throw exception;
            }
        }
        ...
        private void Mail(SMTPContext context, string argument)
        {
            ...
            if (context.LastCommand == COMMAND_HELO)
            {
                ...
                if (!addressValid)
                {
                    Trace.WriteLine(string.Format("Connection {0}: MailFrom argument: {1} rejected.  Should be from:<username@domain.com>", context.ConnectionId, argument));
        ...
        private void Rcpt(SMTPContext context, string argument)
        {
            if (context.LastCommand == COMMAND_MAIL || context.LastCommand == COMMAND_RCPT)
            {
                ...
                if (address != null)
                {
                    ...
                    {
                        Trace.WriteLine(string.Format("Connection {0}: RcptTo argument: {1} rejected.  Should be from:<username@domain.com>", context.ConnectionId, argument));
                        context.WriteLine(MESSAGE_INVALID_ADDRESS);
                    }
                }
                else
